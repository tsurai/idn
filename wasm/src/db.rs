use chrono::prelude::*;
use js_sys::{Promise, wasm_bindgen::JsCast};
use rs_fsrs::Card;
use wasm_bindgen::{JsValue, closure::Closure};
use wasm_bindgen_futures::JsFuture;
use web_sys::{
    DomException, Event, IdbCursorWithValue, IdbDatabase, IdbKeyRange, IdbObjectStore,
    IdbObjectStoreParameters, IdbRequest,
};
...
// Increment whenever either the structure or initial data of the database changes
const VERSION: u32 = 2;
...
impl IdxDb {
    pub async fn new() -> Result<IdxDb, JsValue> {
        let request = if crate::Idn::is_worker() {
            js_sys::global()
        ...
        Ok(IdxDb { db: Rc::new(db) })
    }

    fn init(db: &IdbDatabase) -> Result<(), DomException> {
        let parameters = IdbObjectStoreParameters::new();
        parameters.set_key_path(&"id".into());
        let store = db.create_object_store_with_optional_parameters("vocabs", &parameters)?;
        store.create_index_with_str("vocab", "vocab")?;
        store.create_index_with_str_sequence(
            "idx_lesson",
            &js_sys::Array::of2(&"seen".into(), &"lesson".into()).into(),
        )?;
        ...
        let parameters = IdbObjectStoreParameters::new();
        parameters.set_key_path(&"id".into());
        let store = db.create_object_store_with_optional_parameters("sentences", &parameters)?;
        store.create_index_with_str("vocabs", "vocabs")?;
    ...
    async fn upgrade(db: &IdbDatabase) -> Result<(), JsValue> {
        ...
        let transaction =
            db.transaction_with_str_and_mode("vocabs", web_sys::IdbTransactionMode::Readwrite)?;
        ...
        let vocab_store = transaction.object_store("vocabs")?;
        ...
        for line in content.lines() {
            ...
            js_sys::Reflect::set(&vocab_obj, &"id".into(), &id)?;
            ...
            for attr in ["lesson", "vocab", "meaning", "pos", "detail", "sentences"] {
                let val = fields
                    ...
                    .unwrap_or(JsValue::NULL);
                js_sys::Reflect::set(&vocab_obj, &attr.into(), &val)?;
            }

            js_sys::Reflect::set(&vocab_obj, &"seen".into(), &0.into())?;

            Self::add_or_update(&vocab_store, &vocab_obj, |prev| {
                let new = vocab_obj.clone();
                js_sys::Reflect::set(
                    &new,
                    &"seen".into(),
                    &js_sys::Reflect::get(prev, &"seen".into()).expect("seen value"),
                )
                .expect("updated vocab value");
                *prev = new.into();
            })
            .await?;
        }
    ...
    pub async fn get_review_card(&self) -> Result<Option<(Card, Vocab)>, JsValue> {
            ...
            .and_then(|x| x.dyn_into::<IdbCursorWithValue>())
        {
            ...
            let vocab = self.get_vocab_by_id(id).await?.ok_or("invalid vocab id")?;
            Ok(Some((card, vocab)))
        } else {
            Ok(None)
        }
    ...
    pub async fn get_review_stats(&self) -> Result<(Option<DateTime<Utc>>, usize), JsValue> {
        ...
        let request = index.count_with_key(&range.clone().into())?;
        ...
        if count == 0 {
            ...
            let cursor = Self::do_request(&request).await?;

            if cursor.is_null() {
                Ok((None, 0))
            } else {
                let obj = cursor.dyn_into::<IdbCursorWithValue>()?.value()?;
    ...
    pub async fn add_sentences(&self, word: &str) -> Result<(), JsValue> {
        let sentences = utils::fetch_tatoeba(word, 10, 50).await?;

        let store = self
            .db
            .transaction_with_str_and_mode("sentences", web_sys::IdbTransactionMode::Readwrite)?
            .object_store("sentences")?;

        for s in js_sys::Array::from(&sentences) {
            utils::clean_sentence(&s)?;
            js_sys::Reflect::set(&s, &"vocabs".into(), &js_sys::Array::of1(&word.into()))?;

            Self::add_or_update(&store, &s, |val| {
                crate::console_log!("updating: {val:?}");
                let vocabs_obj = js_sys::Reflect::get(val, &"vocabs".into()).unwrap();
                let array = vocabs_obj.dyn_ref::<js_sys::Array>().unwrap();
                array.push(&word.into());
                crate::console_log!("update: {vocabs_obj:?}\n{val:?}");
            })
            .await?;
        }
    ...
    async fn add_or_update<F>(
        store: &IdbObjectStore,
        value: &JsValue,
        fn_update: F,
    ) -> Result<(), JsValue>
    where
        F: FnOnce(&mut JsValue),
    {
        let id = js_sys::Reflect::get(value, &"id".into())?;
        let get_request = store.get(&id)?;
        match Self::do_request(&get_request).await {
            Ok(mut curr) if curr != JsValue::UNDEFINED => {
                fn_update(&mut curr);
                let put_request = store.put(&curr)?;
                Self::do_request(&put_request).await?;
            }
            _ => {
                let add_request = store.add(value)?;
                Self::do_request(&add_request).await?;
            }
        }
        ...
        let promise = Promise::new(&mut |resolve, reject| {
            ...
            let cb_success = Closure::<dyn Fn(_)>::new(move |evt: Event| {
                    ...
                    .and_then(|x| x.result().ok())
                {
                    ...
                    reject_
                        .call1(&JsValue::NULL, &"invalid query result".into())
                        .ok();
                }
            });
            request.set_onsuccess(Some(cb_success.as_ref().unchecked_ref()));
    ...
    pub async fn create_card(&self, id: usize) -> Result<String, JsValue> {
        let get_request = self
            .db
            .transaction_with_str("vocabs")?
            .object_store("vocabs")?
        ...
        let obj = Self::do_request(&get_request).await?;
        let vocab = js_sys::Reflect::get(&obj, &"vocab".into())?
            .as_string()
            .expect("valid string");
        ...
        let transaction = self
            .db
            .transaction_with_str_and_mode("cards", web_sys::IdbTransactionMode::Readwrite)?;
        let card_store = transaction.object_store("cards")?;

        let card_obj = utils::card_to_object(rs_fsrs::Card::new())?;

        js_sys::Reflect::set(&card_obj, &"id".into(), &(id as f64).into())?;

        Self::do_request(&card_store.add(&card_obj)?).await?;
